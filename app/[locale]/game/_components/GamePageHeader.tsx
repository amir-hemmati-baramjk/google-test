...
import { getChangeTurn } from "@/core/game/change-turn-service"; // Adjust the import path
import { useGameStore } from "@/stores/gameStore";
import { toast } from "react-toastify";

// Dynamically import GameExitModal with SSR disabled
      ...
      <div className="bg-primary-bg-gradient text-white flex justify-between items-center px-2 md:px-5 py-2">
        ...
        <div className="min-w-[150px]">
          <button
            ...
            className="bg-white rounded-[10px] flex justify-center items-center px-3  font-[700] text-primary text-nowrap text-xs md:text-md lg:text-lg disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <p>
              Team role:{" "}
          ...
          {changeTurnMutation.isPending && (
            <p className="text-xs text-white mt-1">Changing turn...</p>
          )}
        </div>
      </div>
      <GameExitModal
